...
from Agent.modules.agent_perception import TaskPerceptionResult, SQLPerceptionResult
...
async def generate_plan(
    problem_type: str,
    perception: Union[TaskPerceptionResult, SQLPerceptionResult],
...
Final Answer can NEVER be an intermediate step. Use the tools provided at your disposal to solve any intermediate step or to achieve
an SQL QUERY. This is very important.

Respond in **exactly two lines** using one of the following formats:

- FIRST FORMAT:
    - FUNCTION_CALL: tool_name|param1=value1|param2=value2|param3=PREVIOUS_TOOL_RESULT

-  SECOND FORMAT:
    - FINAL_ANSWER: [your final result] *(Not description, but actual final answer)

Always check if you have arrived at the answer for the user-query. YOUR answer CANNOT be the steps to be taken to acheive the result.
Respond EXACTLY with ONE OF THE ABOVEMENTIONED FORMATS. Nothing EXTRA IS REQUIRED.
...
- Current_Step_Perception: {perception.current_step_perception}
...
- FUNCTION_CALL: inner_join|table_name="name"...|df_str=PREVIOUS_TOOL_RESULT
